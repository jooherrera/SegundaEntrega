  ...
  async save(object) {
    try {
      ...
      if(res.ok){ // Si se pudo leer el archivo.
        ...
        dataObject.push({ id: id, ...newObject }); //Se hace un push con el objeto nuevo.
      ...
      fs.promises.writeFile(`./src/models/${this.fileName}`, JSON.stringify([{ id: 1, ...newObject }]),error => {throw error})
        ...
        if(res.ok){ // Si se pudo leer el archivo.
          const mapeado = data.map((el) => {
             if (el.id == id){
                ...
                el.descripcion = object.descripcion
                el.codigo = object.codigo
                el.stock = object.stock
               return el
              }else{
                return el
              }
